...
public class ImageRecognizer2 {
	...
	public List<Contour> getContours(double[] data){
		
		countGray();
		if(data[1]>0)
			sharpen(imageGray,(int)data[0],(int)data[1]); 
        countBinary(imageGray,data[2],data[3]);
        closeMorph(imageBinary,(int)data[4],(int)data[5]);
   //     Imgproc.cvtColor(image, imageHLS, Imgproc.COLOR_BGR2HLS);
	...
	private void countBinary(Mat img, double thresh1, double thresh2){
		imageBinary = new Mat();
        Imgproc.Canny(img, imageBinary, thresh1, thresh2);
	...
	private void sharpen(Mat img, int sigma,int times){
		Mat blurred = new Mat();
        Imgproc.GaussianBlur(img, blurred, new Size(0,0), sigma);
